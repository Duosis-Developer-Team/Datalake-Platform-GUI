...
surface the actual U quantities (free/used/total) rather than collapsing to
the "CRM entitled — infra telemetry pending" placeholder that applies when
infra data is absent.
"""
...
def _dc_hosting_u_row(**overrides):
    """A populated dc_hosting_u row, matching the real backend shape.
...
def test_dc_hosting_u_row_surfaces_u_quantities_when_infra_present():
    """Populated infra (has_infra_source=True) -> real U quantities, no pending hint."""
    ...
    # Free and Used carry the real U figures (not em-dash placeholders).
    assert comp["free_fmt"] == "1,799 U\n—"
    assert comp["used_fmt"] == "1,817 U\n—"
    assert comp["total_fmt"] == "3,616 U"
    assert comp["display_unit"] == "U"
    assert comp["has_infra_source"] is True

    # Sanity: these would fail if free/used got swapped or the row were
    # mishandled as a crm-only / infra-absent row.
    assert "1,799" in comp["free_fmt"]
    assert "1,817" in comp["used_fmt"]
    assert "1,799" not in comp["used_fmt"]
