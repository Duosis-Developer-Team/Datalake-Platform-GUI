...
class TestGetGlobalDashboard(unittest.TestCase):
    ...
    def test_returns_overview_and_platforms(self):
        ...
        cache.set(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}", {
            ...
            "compute": {
                "classic":   {"hosts": 10, "vms": 50},
                "hyperconv": {"hosts": 30, "vms": 150},
                "power":     {"hosts": 5,  "vios": 2, "lpars": 20},
            },
    ...
    def test_compute_key_present_in_cached_dashboard(self):
        tr = default_time_range()
        cache.set(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}", {
            "overview": {},
            "platforms": {},
        ...
        self.assertIn("classic",   result["compute"])
        self.assertIn("hyperconv", result["compute"])
        self.assertIn("power",     result["compute"])

    def test_compute_classic_values(self):
        tr = default_time_range()
        cache.set(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}", {
            "overview": {},
            "platforms": {},
            "compute": {
                "classic":   {"hosts": 12, "vms": 60},
                "hyperconv": {"hosts": 20, "vms": 100},
                "power":     {"hosts": 3,  "vios": 1, "lpars": 15},
            },
            "energy_breakdown": {},
        })
        result = self.svc.get_global_dashboard()
        self.assertEqual(result["compute"]["classic"]["hosts"], 12)
        self.assertEqual(result["compute"]["classic"]["vms"],   60)
        self.assertEqual(result["compute"]["hyperconv"]["hosts"], 20)
        self.assertEqual(result["compute"]["hyperconv"]["vms"],   100)
        self.assertEqual(result["compute"]["power"]["hosts"],  3)
        self.assertEqual(result["compute"]["power"]["vios"],   1)
        self.assertEqual(result["compute"]["power"]["lpars"],  15)


# ---------------------------------------------------------------------------
# Compute architecture global aggregation — _rebuild_summary
# ---------------------------------------------------------------------------

class TestComputeArchitectureAggregation(unittest.TestCase):
    """Verifies that _rebuild_summary aggregates classic/hyperconv/power
    compute totals into the global_dashboard cache under the 'compute' key."""

    def setUp(self):
        cache.clear()
        self.svc = _make_service()

    def _make_dc_with_compute(self, dc_code, cl_hosts, cl_vms, hc_hosts, hc_vms,
                               ibm_hosts=0, ibm_vios=0, ibm_lpars=0):
        d = _EMPTY_DC(dc_code)
        d["classic"]["hosts"]   = cl_hosts
        d["classic"]["vms"]     = cl_vms
        d["hyperconv"]["hosts"] = hc_hosts
        d["hyperconv"]["vms"]   = hc_vms
        d["platforms"]["ibm"]["hosts"] = ibm_hosts
        d["platforms"]["ibm"]["vios"]  = ibm_vios
        d["platforms"]["ibm"]["lpars"] = ibm_lpars
        d["intel"]["hosts"] = cl_hosts + hc_hosts + ibm_hosts
        d["intel"]["vms"]   = cl_vms + hc_vms
        return d

    def _run_rebuild(self, all_dc_data):
        platform_counts = {dc: 1 for dc in all_dc_data}

        def _fake_fetch(cursor, dc_list, start_ts, end_ts):
            return all_dc_data, platform_counts

        with patch.object(self.svc, "_fetch_all_batch", side_effect=_fake_fetch), \
             patch.object(self.svc, "_load_dc_list", return_value=list(all_dc_data.keys())):
            self.svc._rebuild_summary(default_time_range())

    def test_compute_key_written_to_cache(self):
        dc = self._make_dc_with_compute("DC11", cl_hosts=5, cl_vms=20, hc_hosts=10, hc_vms=40)
        self._run_rebuild({"DC11": dc})
        tr = default_time_range()
        cached = cache.get(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}")
        self.assertIsNotNone(cached)
        self.assertIn("compute", cached)

    def test_classic_totals_aggregated_across_dcs(self):
        dc1 = self._make_dc_with_compute("DC11", cl_hosts=5,  cl_vms=20, hc_hosts=10, hc_vms=40)
        dc2 = self._make_dc_with_compute("DC12", cl_hosts=15, cl_vms=60, hc_hosts=20, hc_vms=80)
        self._run_rebuild({"DC11": dc1, "DC12": dc2})
        tr = default_time_range()
        cached = cache.get(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}")
        self.assertEqual(cached["compute"]["classic"]["hosts"], 20)   # 5+15
        self.assertEqual(cached["compute"]["classic"]["vms"],   80)   # 20+60

    def test_hyperconv_totals_aggregated_across_dcs(self):
        dc1 = self._make_dc_with_compute("DC11", cl_hosts=5,  cl_vms=20, hc_hosts=10, hc_vms=40)
        dc2 = self._make_dc_with_compute("DC12", cl_hosts=15, cl_vms=60, hc_hosts=20, hc_vms=80)
        self._run_rebuild({"DC11": dc1, "DC12": dc2})
        tr = default_time_range()
        cached = cache.get(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}")
        self.assertEqual(cached["compute"]["hyperconv"]["hosts"], 30)   # 10+20
        self.assertEqual(cached["compute"]["hyperconv"]["vms"],   120)  # 40+80

    def test_power_totals_mirror_ibm_platforms(self):
        dc = self._make_dc_with_compute(
            "DC11", cl_hosts=5, cl_vms=20, hc_hosts=10, hc_vms=40,
            ibm_hosts=3, ibm_vios=2, ibm_lpars=12,
        )
        self._run_rebuild({"DC11": dc})
        tr = default_time_range()
        cached = cache.get(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}")
        self.assertEqual(cached["compute"]["power"]["hosts"], 3)
        self.assertEqual(cached["compute"]["power"]["vios"],  2)
        self.assertEqual(cached["compute"]["power"]["lpars"], 12)

    def test_compute_zero_when_no_dc_data(self):
        dc = _EMPTY_DC("DC11")
        dc["intel"]["hosts"] = 1   # prevent DC from being filtered out
        self._run_rebuild({"DC11": dc})
        tr = default_time_range()
        cached = cache.get(f"global_dashboard:{tr.get('start','')}:{tr.get('end','')}")
        self.assertEqual(cached["compute"]["classic"]["hosts"],   0)
        self.assertEqual(cached["compute"]["hyperconv"]["hosts"], 0)
