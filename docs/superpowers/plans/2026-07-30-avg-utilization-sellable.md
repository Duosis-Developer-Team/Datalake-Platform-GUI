...
- **Known and accepted gap — do not widen the diff to fix it.** In `constrain_by_ratio_per_host_dual`'s generic branch, a storage panel falls through to `replace(p, sellable_constrained=p.sellable_raw, ratio_bound=False)`, so it receives neither `sellable_max_util` nor `sellable_avg_util` and both stay `None`. This predates the branch — `sellable_max_util` had the identical gap — and it renders an em-dash in *both* columns, which reads consistently rather than as "a real number beside a missing figure". Surfaced by the Task 5 review; left alone deliberately.
...
In `shared/sellable/host_sellable.py`, first add two module-level helpers above `host_raw_headroom` — one presence test, and one extracted max-track computation per resource. The max helpers keep their existing `or` chains verbatim so max-track numbers do not shift beyond the intended CPU peak correction:
...
Then replace the `cpu` and `ram` blocks of `host_raw_headroom` with:
