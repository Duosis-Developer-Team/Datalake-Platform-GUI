...
- **Missing average data must never become `0`.** A zero reads as "nothing is used, sell everything". Enrichers no-op on empty query results, and `sellable_avg_util` stays `None` so the GUI renders `—`.
