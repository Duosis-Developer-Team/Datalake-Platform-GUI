...
def host_storage_free_gb(host: dict, *, include_shared: bool) -> float:
    """Return gated storage headroom for a host (GB).

    ``include_shared=False`` counts only exclusive datastore free.
    ``include_shared=True`` adds all shared pool free visible to this host.
    """
    ...
    # Hyperconverged local disk (no mounts): use stor_free_gb directly.
...
def host_raw_headroom(
    ...
    *,
    ...
    if resource == "cpu":
        ghz = float(host.get("ghz_per_core") or 1.0)
        cap = float(host.get("cpu_cap_ghz") or 0.0)
        if cpu_track == "physical":
            alloc = float(host.get("cpu_alloc_ghz_physical") or 0.0)
        else:
            cap = cap  # display cap stays GHz
            alloc = float(host.get("cpu_alloc_ghz") or 0.0)
        util = float(host.get("cpu_used_pct") or 0.0)
        return apply_utilization_gate(cap, alloc, util, threshold_pct)

    if resource == "ram":
        cap = float(host.get("mem_cap_gb") or 0.0)
        if ram_track == "peak":
            cap = float(host.get("mem_cap_gb_at_peak") or host.get("mem_peak_total") or cap)
        ...
        alloc = float(host.get("mem_alloc_gb") or 0.0)
        util = float(host.get("mem_used_pct") or 0.0)
        return apply_utilization_gate(cap, alloc, util, threshold_pct)
...
def compute_host_sellable_units(
    ...
    *,
    ...
    if cap > 0:
        ...
        raw_stor = min(raw_stor_gate, stor_free_min) if stor_free_min > 0 else 0.0
        raw_stor_max = min(raw_stor_gate, stor_free_max) if stor_free_max > 0 else 0.0
    else:
        raw_stor = 0.0
        raw_stor_max = stor_free_max

    if cap > 0 and raw_stor <= 0:
        n_min = 0.0
    else:
...
def aggregate_family_storage_range(
    ...
    Min = sum of per-host min storage constrained (exclusive-only attribution).
    Max = min(total shared free deduped, best host max scenario) summed with
    exclusive mins — conservative cap on double-counting shared pools.
    """
    if not host_results:
        return 0.0, 0.0
