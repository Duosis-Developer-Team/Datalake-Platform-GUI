...
def _customer_content(customer_name: str, time_range: dict | None = None):
    ...
    # Fallback: legacy totals without category split still show under Application
            ...
            (
                ...
                _tab_netbackup_category(
                    ...
                    crm_eff_panel=build_sold_vs_used_stack(
                        filter_efficiency_rows(eff_by_cat, "backup.netbackup")
                    ),
    ...
    if has_veeam or has_zerto:
        ...
        if has_veeam:
            repl_children.append(
                _tab_veeam(
                    ...
                    backup_totals,
                    crm_eff_panel=build_sold_vs_used_stack(
                        filter_efficiency_rows(eff_by_cat, "backup.veeam")
                    ),
                )
            )
            veeam_lic = _crm_license_panel_from_efficiency(eff_by_cat, "licensing.veeam")
            if veeam_lic is None:
                # Also try backup.veeam license-like rows / sold-only note
        ...
        if has_zerto:
            repl_children.append(
                _tab_zerto(
                    ...
                    backup_totals,
                    crm_eff_panel=build_sold_vs_used_stack(
                        filter_efficiency_rows(eff_by_cat, "backup.zerto")
                    ),
                )
            )
        ...
        backup_tab_defs.append(
            (
                "replication",
                "Replication",
                dmc.Stack(gap="lg", children=repl_children),
            )
        )
