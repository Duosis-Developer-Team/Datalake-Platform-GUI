...
# Possessive: Boyner'in, Akbank'ın (case-insensitive name capture)
_POSSESSIVE_RE = re.compile(
    r"\b([\wÇĞİÖŞÜçğıöşü&.\-]{2,})['’](?:in|ın|nin|nın|un|ün|nun|nün)\b",
    re.IGNORECASE,
)
# "Boyner müşterisi", "aselsan müşterisi", "Akbank firması"
_CUSTOMER_LABEL_RE = re.compile(
    r"\b([\wÇĞİÖŞÜçğıöşü&.\-]{2,})\s+"
    ...
    *,
    ...
    for candidate in candidates:
        for name in catalog:
            ...
            cf = _fold(candidate)
            nf = _fold(name)
            if cf == nf or (cf and nf and (cf in nf or nf in cf)):
...
def resolve_customer_name(
    ...
    *,
    ...
    if selected_customer:
        if catalog_payload is not None:
            matched = fuzzy_match_customer(selected_customer, catalog_payload, min_score=0.6)
            return matched or selected_customer
...
def normalize_customer_tool_args(
    args: dict[str, Any],
    resolved_name: str,
) -> dict[str, Any]:
    """Rewrite customer_name on tool args when a canonical CRM name is known."""
    out = dict(args)
    if resolved_name:
        out["customer_name"] = resolved_name
    return out
