...
METRICS: dict[str, MetricDefinition] = {
    ...
    "customer_overview": MetricDefinition(
        key="customer_overview",
        aliases=(
            "genel bilgi", "ne biliyorsun", "hakkında bilgi", "genel durum",
            "müşteri özeti", "musteri ozeti", "customer overview", "durumu analiz",
            "hakkında özet", "hakkında genel",
        ),
        entity="customer", metric="overview", calculation="summary", output_type="summary",
        ...
        required_params=(),
        answer_guidance=(
            "Lead with CRM/commercial summary (orders, revenue, ITSM, compliance) before infrastructure.",
            "If get_customer_resources timed out, still report catalog and sales/compliance data.",
            "Never claim the customer has no data when catalog or sales tools returned values.",
        ),
        explanation="Genel müşteri sorusu — ticari/CRM ve altyapı özeti (CRM öncelikli).",
    ),
    "customer_sales_summary": MetricDefinition(
        key="customer_sales_summary",
        aliases=("satış özeti", "satis ozeti", "sales summary", "ytd gelir", "ytd revenue",
                 "aktif sipariş", "aktif siparis", "active order", "crm satış", "crm satis"),
        entity="customer", metric="sales", calculation="summary", output_type="summary",
        analysis_profile="generic",
        primary_tools=("get_customer_sales_summary",),
        fallback_tools=("get_customer_sales_active_orders", "get_customer_efficiency_by_category"),
        required_params=("customer_name",),
        explanation="Müşteri CRM satış özeti ve aktif siparişler.",
    ),
    "customer_itsm_risk": MetricDefinition(
        key="customer_itsm_risk",
        aliases=("itsm", "ticket özeti", "ticket ozeti", "çağrı", "cagri", "incident",
                 "destek talebi", "müşteri ticket", "musteri ticket"),
        entity="customer", metric="itsm", calculation="risk", output_type="risk_analysis",
        analysis_profile="generic",
        primary_tools=("get_customer_itsm_summary",),
        fallback_tools=("get_customer_itsm_extremes", "get_customer_itsm_tickets"),
        required_params=("customer_name",),
        explanation="Müşteri ITSM ticket özeti ve risk sinyalleri.",
    ),
    "customer_compliance": MetricDefinition(
        key="customer_compliance",
        aliases=("resource compliance", "uyum", "compliance", "sold vs used",
                 "satılan kullanılan", "satilan kullanilan", "verimlilik kategori",
                 "efficiency by category"),
        entity="customer", metric="compliance", calculation="comparison", output_type="comparison",
        analysis_profile="generic",
        primary_tools=("get_customer_resource_compliance",),
        fallback_tools=("get_customer_efficiency_by_category",),
        required_params=("customer_name",),
        explanation="Müşteri CRM entitlement vs kullanım uyumu.",
    ),
    ...
    "crm_sellable": MetricDefinition(
        ...
        answer_guidance=(
            "When the user names a datacenter (e.g. DC13), scope sellable tools to that dc_code.",
            "Global sellable questions use dc_code=* only when no DC is specified.",
        ),
