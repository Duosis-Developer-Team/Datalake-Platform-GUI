...
WITH agg AS (
    SELECT vmname,
        COALESCE(MIN(
            CASE WHEN COALESCE(total_cpu_capacity_mhz, 0) > 0
            THEN 100.0 * cpu_usage_min_mhz / total_cpu_capacity_mhz ELSE 0 END
        ), 0) AS cpu_pct_min,
        COALESCE(AVG(
            CASE WHEN COALESCE(total_cpu_capacity_mhz, 0) > 0
            THEN 100.0 * cpu_usage_avg_mhz / total_cpu_capacity_mhz ELSE 0 END
        ), 0) AS cpu_pct_avg,
        COALESCE(MAX(
            CASE WHEN COALESCE(total_cpu_capacity_mhz, 0) > 0
            THEN 100.0 * cpu_usage_max_mhz / total_cpu_capacity_mhz ELSE 0 END
        ), 0) AS cpu_pct_max,
