...
VALUES
    ...
    -- allocated_unit reflects Redis dc_details.hyperconv field units (GHz/GB/TB)
    ('virt_hyperconverged_cpu', '*',
        'nutanix_cluster_metrics',  'total_cpu_capacity',    'Hz',
        'nutanix_vm_metrics',       'cpu_count',             'GHz',
        'datacenter_name ILIKE :dc_pattern',
        'Nutanix cluster CPU total from DB; allocated from Redis hyperconv.cpu_used (GHz).', 'seed'),
    ('virt_hyperconverged_ram', '*',
        'nutanix_cluster_metrics',  'total_memory_capacity', 'bytes',
        'nutanix_vm_metrics',       'memory_capacity',       'GB',
        'datacenter_name ILIKE :dc_pattern',
        'Nutanix cluster memory total from DB; allocated from Redis hyperconv.mem_used (GB).', 'seed'),
    ('virt_hyperconverged_storage', '*',
        'nutanix_cluster_metrics',  'storage_capacity',      'bytes',
        'nutanix_vm_metrics',       'disk_capacity',         'TB',
        'datacenter_name ILIKE :dc_pattern',
        'Nutanix storage total from DB; allocated from Redis hyperconv.stor_used (TB).', 'seed'),

    -- Klasik Mimari (VMware) — source_table datacenter_metrics (SellableService applies latest-per-(dc,datacenter) like vmware.py)
    -- allocated_unit reflects Redis dc_details.classic field units (GHz/GB/TB)
    ('virt_classic_cpu', '*',
        'datacenter_metrics', 'total_cpu_ghz_capacity', 'GHz',
        'vm_metrics',         'number_of_cpus',          'GHz',
        'datacenter ILIKE :dc_pattern',
        'CPU/RAM/storage totals from datacenter_metrics; allocated from Redis classic.cpu_used (GHz).', 'seed'),
    ('virt_classic_ram', '*',
        'datacenter_metrics', 'total_memory_capacity_gb', 'GB',
        'vm_metrics',         'total_memory_capacity_gb', 'GB',
        'datacenter ILIKE :dc_pattern',
        'Allocated from Redis classic.mem_used (GB).', 'seed'),
    ('virt_classic_storage', '*',
        'datacenter_metrics', 'total_storage_capacity_gb', 'GB',
        'vm_metrics',         'provisioned_space_gb', 'TB',
        'datacenter ILIKE :dc_pattern',
        'Total in GB from datacenter_metrics; allocated from Redis classic.stor_used (TB → GB via conversion).', 'seed'),

    -- Classic KM clusters — source_table cluster_metrics (SellableService applies latest-per-(cluster,datacenter))
    -- allocated_unit reflects Redis dc_details.classic field units (GHz/GB/TB)
    ('virt_km_cpu', '*',
        'cluster_metrics', 'cpu_ghz_capacity', 'GHz',
        'vm_metrics',      'number_of_cpus',   'GHz',
        'datacenter ILIKE :dc_pattern AND cluster ILIKE ''%KM%''',
        'KM rows only; same grain as vmware.py CLASSIC_METRICS. Allocated from Redis classic.cpu_used (GHz).', 'seed'),
    ('virt_km_ram', '*',
        'cluster_metrics', 'memory_capacity_gb', 'GB',
        'vm_metrics',      'total_memory_capacity_gb', 'GB',
        'datacenter ILIKE :dc_pattern AND cluster ILIKE ''%KM%''',
        'Allocated from Redis classic.mem_used (GB).', 'seed'),
    ('virt_km_storage', '*',
        'cluster_metrics', 'total_capacity_gb', 'GB',
        'vm_metrics',      'provisioned_space_gb', 'TB',
        'datacenter ILIKE :dc_pattern AND cluster ILIKE ''%KM%''',
        'Total in GB from cluster_metrics; allocated from Redis classic.stor_used (TB → GB via conversion).', 'seed'),
