"""OpenTelemetry SDK for datacenter-api (OTLP gRPC)."""
...
def _resource_attrs() -> dict:
    name = (os.environ.get("OTEL_SERVICE_NAME") or "datacenter-api").strip()
...
def setup_sdk() -> None:
    """Initialize tracing and auto-instrumentation. Call once at process startup."""
    global _sdk_initialized
    if _sdk_initialized:
        return
    _sdk_initialized = True
    if not _enabled():
        _logger.info("OpenTelemetry disabled for datacenter-api")
        return
    ...
    _logger.info("OpenTelemetry enabled datacenter-api endpoint=%s", endpoint)
