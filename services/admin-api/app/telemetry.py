"""OpenTelemetry SDK for admin-api (OTLP gRPC)."""
...
def _resource_attrs() -> dict:
    name = (os.environ.get("OTEL_SERVICE_NAME") or "admin-api").strip()
...
def setup_sdk() -> None:
    ...
    if not _enabled():
        _logger.info("OpenTelemetry disabled for admin-api")
        return
    ...
    _logger.info("OpenTelemetry enabled admin-api endpoint=%s", endpoint)
