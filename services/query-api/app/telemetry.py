"""OpenTelemetry SDK for query-api (OTLP gRPC)."""
...
def _resource_attrs() -> dict:
    name = (os.environ.get("OTEL_SERVICE_NAME") or "query-api").strip()
...
def setup_sdk() -> None:
    ...
    if not _enabled():
        _logger.info("OpenTelemetry disabled for query-api")
        return
    ...
    _logger.info("OpenTelemetry enabled query-api endpoint=%s", endpoint)
