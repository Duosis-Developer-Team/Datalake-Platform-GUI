"""OpenTelemetry SDK for hmdl-api (OTLP gRPC)."""
...
def _resource_attrs() -> dict:
    name = (os.environ.get("OTEL_SERVICE_NAME") or "hmdl-api").strip()
...
def setup_sdk() -> None:
    ...
    if not _enabled():
        _logger.info("OpenTelemetry disabled for hmdl-api")
        return
    try:
        ...
        _logger.warning("OpenTelemetry packages unavailable: %s", exc)
        return
    ...
    _logger.info("OpenTelemetry enabled hmdl-api endpoint=%s", endpoint)
